...
app.post('/submit', (req, res) => {
    ...
    doc.fontSize(16).text('Hours, Miles, and Time Spent Driving Each Day', { align: 'left' }).moveDown();

    const daysOfWeek = ['Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday'];

    daysOfWeek.forEach(day => {
        const dayLower = day.toLowerCase();
        if (data[`${dayLower}StartTime`] && data[`${dayLower}EndTime`]) {
            doc.fontSize(14).text(`${day}:`, { align: 'left', continued: true });
            doc.fontSize(14).text(` ${data[`${dayLower}StartTime`]} to ${data[`${dayLower}EndTime`]},`, { align: 'left', continued: true });
            doc.fontSize(14).text(` ${data[`${dayLower}Miles`]} mile(s),`, { align: 'left', continued: true });
            doc.fontSize(14).text(` ${data[`${dayLower}DrivingTime`]} minute(s) driving,`, { align: 'left', continued: true });
            doc.fontSize(14).text(` ${data[`${dayLower}ServiceType`]} service,`, { align: 'left', continued: true });
            doc.fontSize(14).text(` patient(s): ${data[`${dayLower}PatientNames`]}`, { align: 'left' }).moveDown();
        }
    });

    // Add totals
    doc.fontSize(16).text('Totals:', { align: 'left' }).moveDown();
    doc.fontSize(14).text(`Total Hours: ${data.totalHours}`, { align: 'left' }).moveDown();
...
app.post('/send-email', async (req, res) => {
    ...
    const recipientEmails = [email, 'joshuaschoen@outlook.com','lbattle@AffinityHospice.com'];
